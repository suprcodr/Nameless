 ...
 * @Support <http://www.almsaeedstudioptions.com>
 * @Email   <abdullah@almsaeedstudioptions.com>
 * @version 2.3.8
...
$.AdminLTE.options = {
    ...
    enableBootstrapToolTip: true,
    bootstrapToolTipSelector: "[data-toggle='tooltip']",
...
$(function () {
    ...
    if (options.enableBootstrapToolTip) {
        $('body').tooltip({
            selector: options.bootstrapToolTipSelector
        });
    }
...
function initialize() {
    ...
    $.AdminLTE.controlSidebar = {
        ...
        _fix: function (sidebar) {
            ...
            if ($("body").hasClass('layout-boxed')) {
                ...
                if (self.hasBindedResize) {
                    return;
                }
                $(window).resize(function () {
                    self._fix(sidebar);
                });
                self.hasBindedResize = true;
            } else {
                sidebar.css({
                    'position': 'fixed',
                    'height': 'auto'
    ...
    $.AdminLTE.boxWidget = {
        ...
        activate: function (_box) {
            ...
            $(_box).on('click', self.selectors.collapse, function (e) {
                e.preventDefault();
                self.collapse($(this));
            });

            //Listen for remove event triggers
            $(_box).on('click', self.selectors.remove, function (e) {
                e.preventDefault();
                self.remove($(this));
            });
        },
